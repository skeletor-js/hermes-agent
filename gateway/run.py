...
class GatewayRunner:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        # Model/provider change invalidates all cached agents — the config
        # signature will differ, but eagerly clearing avoids stale cache hits
        # during the race window before the next message resolves.
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        # Reasoning change invalidates cached agents
        _lock = getattr(self, "_agent_cache_lock", None)
        if _lock:
            with _lock:
                self._agent_cache.clear()
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            # Per-message callbacks — these change every turn and must not be
            # baked into the cached agent constructor.
